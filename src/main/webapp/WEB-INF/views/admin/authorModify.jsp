...
<body>
                    	...
                    	<form id="modifyForm" action="/admin/authorModify" method="post">
	                   		...
	                   		<div class="form_section">
                   				...
                   				<select class="input_block" name="genreId" >
	                   					...
	                   					<option value="01"  <c:out value=" ${authorInfo.genreId eq '01' ?'selected':''}"/>>귀여운</option>
	                   					<option value="02"  <c:out value=" ${authorInfo.genreId eq '02' ?'selected':''}"/>>신비로운</option>
	                   					<option value="03"  <c:out value=" ${authorInfo.genreId eq '03' ?'selected':''}"/>>웅장한</option>
	                   					<option value="04"  <c:out value=" ${authorInfo.genreId eq '04' ?'selected':''}"/>>무서운</option>
	                   					<option value="05"  <c:out value=" ${authorInfo.genreId eq '05' ?'selected':''}"/>>자연의</option>
	                   					<option value="06"  <c:out value=" ${authorInfo.genreId eq '06' ?'selected':''}"/>>우주의</option>
	                   				</select>
	                   			</div>
	                   		</div>
	                   		<div class="form_section">
                ...
                <form id="moveForm" method="get">
                	...
                	<input type="hidden" name="amount" value='<c:out value="${cricriteria.amount }"/>' >
                	<input type="hidden" name="keyword" value='<c:out value="${cricriteria.keyword }"/>'>
                </form>
